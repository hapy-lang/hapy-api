...
def create_example(
    db: Session, item: schemas.BiteCreate, user_id: int, challenge_id: int
):
    db_example = models.Solution(
        **item.dict(), owner_id=user_id, challenge_id=challenge_id
    )
    db.add(db_example)
    db.commit()
...
def create_bite(db: Session, item: schemas.BiteCreate, submitter_id: int, hapy_v: str):
    ...
    db_bite = models.BiteBase(
        **item.dict(), slug=slug, submitter_id=submitter_id, hapy_version=hapy_v
    )
    db.add(db_bite)
    db.commit()
    db.refresh(db_bite)
